...
"""Replay the episodes that FAILED under protocol v2 and draw what the agents actually did.
    ...
    python tools/replay_failed.py --ckpt runs/v20_.../ckpt_best.pt --split hybrid --agents 4 \
        --ir2-dir $IR2_ROOT/comparison/results
...
# matplotlib is an OPTIONAL dependency: it is needed only by this tool, so it is not in
# requirements.txt and is absent from the runtime image. Fail with an instruction rather than a
# traceback — a reviewer running this should be told what to install, not shown a stack.
try:
    import matplotlib  # noqa: E402
    matplotlib.use("Agg")
    import matplotlib.pyplot as plt  # noqa: E402
except ImportError:  # pragma: no cover
    sys.exit("tools/replay_failed.py needs matplotlib (an optional dependency):\n"
