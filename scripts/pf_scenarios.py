...
"""Hand-scripted scenarios for the pathfront teammate belief — one case per page, step by step.
    ...
    python scripts/pf_scenarios.py                 # all scenarios → runs/pf_scenarios/
    python scripts/pf_scenarios.py --only 03       # just one
...
def load_impl(path: Path):
    """Run the bench against a DIFFERENT copy of the belief module (e.g. the frozen reference).
        ...
        python scripts/pf_scenarios.py --impl /tmp/ref.py --tag 00_reference
...
def main() -> None:
    ...
    # Pages written by OTHER tools into the same folder — scripts/pf_real_episode.py puts the
    # real-map/real-policy runs here on purpose, so they are read next to the scenarios instead of
    # living at a URL of their own. Listed last, by filename, with no stats of ours to report.
